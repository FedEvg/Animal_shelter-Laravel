...
@section('content')
    <div class="row">
        ...
        <div class="col-4">
            <form method="POST" action="{{ route('admin.animal.store') }}">
                @csrf
                    ...
                    <select class="form-select" name="breed_id" aria-label="Default select example">
                        @foreach($breeds as $breed)
                            <option value="{{ $breed->id }}"
                                {{ $breed->id == old('breed_id') ? ' selected' : '' }}
                            >{{ $breed->getNameKind($breed->kind_id) . " " . $breed->name }}</option>
                        @endforeach
                    </select>
                </div>
                <p class="text-danger">
                ...
                <div class="form-group">
                    ...
                    <select class="form-select" name="sterilization_id" aria-label="Default select example">
                        @foreach($sterilizations as $id => $sterilization)
                            <option value="{{ $id }}"
                                {{ $id == old('sterilization_id') ? ' selected' : '' }}
                ...
                <div class="form-group">
                    <label>Age</label>
                    <input type="date" class="form-control" name="age" placeholder="Enter data" value="{{ old('age') }}">
                </div>
                <p class="text-danger">
                    @error('age')
                    {{ $message }}
